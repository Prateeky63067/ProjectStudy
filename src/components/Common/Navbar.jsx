...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        {" "}
        <div className="md:hidden text-sm flex  max-w-maxContent items-center justify-between w-11/12">
          {/* logo */}
          <Link>
            <img
              className="md:h-8 md:w-40 h-6 w-32"
              src={logo}
              alt="Logo"
              loading="lazy"
            />
          ...
          >
            ...
            >
                ...
                <li key={index}>
                  {element?.title === "Catalog" ? (
                    <div className="border-2 border-black flex items-center gap-1 group">
                      <p onClick={() => setHover(false)}>{element?.title}</p>
                      <BiRightArrow />
                      <div
                        className={`${
                          hover ? "hidden" : "visible"
                        } invisible absolute left-[120%] top-[14%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px] `}
                      >
                          ...
                          <>
                            {subLinks?.map((subLink, i) => (
                              ...
                              >
                                <p
                                  className={`${
                                    hover ? " group-hover:invisible" : "visible"
                                  }`}
                                  onClick={() => {
                                    setNavVisible(false);
                                    setHover(true);
              ...
              {/* login / signup / dashboard  */}
              {/* <div className="flex flex-col justify-center items-center gap-10 text-3xl mt-10 "> */}
              {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
                <Link to="/dashboard/cart" className="relative">
                  <AiOutlineShoppingCart className="text-2xl text-richblack-100" />
                  {totalItems > 0 && (
              ...
              {token === null && (
                <>
                  <Link to="/login">
                    <button
                      onClick={() => setNavVisible(false)}
                      className="rounded-[8px] text-sm border  border-richblack-700 bg-richblack-800 p-1 text-richblack-100"
                    >
                      Log in
                    </button>
                  </Link>
                  <Link to="/signup">
                    <button
                      onClick={() => setNavVisible(false)}
                      className="rounded-[8px] mx-2 text-sm border border-richblack-700 bg-richblack-800 p-1 text-richblack-100"
                    >
                      Sign up
                    </button>
                  </Link>
